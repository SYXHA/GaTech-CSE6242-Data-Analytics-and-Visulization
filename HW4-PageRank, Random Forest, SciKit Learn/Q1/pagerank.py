...
"""
def author():                                                                                             
        return 'yshao85' # replace sroutray3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903423668 # replace with your GT ID number      
...
#############################################
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            temp = [0.0] * (self.max_node_id + 1)
            for source, target in self.read_edge_file(self.edge_file):

### Implement your code here
#############################################
                temp[target] += scores[source]/self.node_degree[source] 
           
            for idx in range(self.max_node_id + 1):
                new_scores[idx] = (1 - damping_factor) * node_weights[idx] + \
                                 damping_factor * temp[idx]
